...
function SectionHeader({ icon, label, color, isDark }: { icon: React.ReactNode; label: string; color: string; isDark: boolean }) {
  return (
    <div className="flex items-center gap-2 mb-3 pt-1">
      <span className={color}>{icon}</span>
      <h4 className={`text-xs font-bold uppercase tracking-widest ${color}`}>{label}</h4>
      <div className={`flex-1 h-px ${isDark ? 'bg-slate-700/60' : 'bg-gray-300'}`} />
    </div>
  );
}
    ...
    <div className={`${isDark ? 'bg-navy-900 border-slate-700' : 'bg-white border-gray-200'} border rounded-xl`}>
      ...
      <div className={`flex items-center justify-between px-5 py-3 border-b ${isDark ? 'border-slate-700 bg-navy-800/50' : 'border-gray-200 bg-gray-50'}`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className={`text-base font-semibold ${isDark ? 'text-white' : 'text-slate-900'} leading-tight`}>AI Risk Assessment</h3>
            <p className={`text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-600'}`}>OCC/FDIC-style narrative</p>
          </div>
        </div>
        <button
          onClick={onRegenerate}
          className={`text-xs px-3 py-1 ${isDark ? 'bg-slate-700 hover:bg-slate-600 text-slate-300' : 'bg-gray-200 hover:bg-gray-300 text-slate-700'} rounded transition flex items-center gap-1`}
        >
      ...
      <div className="p-5 space-y-5">
        ...
        {/* ── Scenario Overview ── */}
        <SectionHeader icon={<Layers className="w-3.5 h-3.5" />} label="Scenario Overview" color="text-blue-400" isDark={isDark} />
        <div className="grid grid-cols-2 gap-1.5 -mt-2">
          <div className={`${isDark ? 'bg-navy-800/50 border-slate-700/40' : 'bg-gray-50 border-gray-200'} rounded px-3 py-1.5 border`}>
            <div className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-600'} uppercase`}>Business Line</div>
            <div className={`text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{businessLine}</div>
          </div>
          <div className={`${isDark ? 'bg-navy-800/50 border-slate-700/40' : 'bg-gray-50 border-gray-200'} rounded px-3 py-1.5 border`}>
            <div className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-600'} uppercase`}>Product</div>
            <div className={`text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{product}</div>
          </div>
          <div className={`${isDark ? 'bg-navy-800/50 border-slate-700/40' : 'bg-gray-50 border-gray-200'} rounded px-3 py-1.5 border`}>
            <div className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-600'} uppercase`}>Geography</div>
            <div className={`text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{geography}</div>
          </div>
          <div className={`${isDark ? 'bg-navy-800/50 border-slate-700/40' : 'bg-gray-50 border-gray-200'} rounded px-3 py-1.5 border`}>
            <div className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-600'} uppercase`}>Risk Type</div>
            <div className={`text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{riskType}</div>
          </div>
        </div>

        {/* ── Assessment Ratings ── */}
        <SectionHeader icon={<Crosshair className="w-3.5 h-3.5" />} label="Assessment Ratings" color="text-amber-400" isDark={isDark} />
        <div className="grid grid-cols-3 gap-1.5 -mt-2">
          <div className="bg-orange-500/10 rounded px-3 py-2 border border-orange-500/20 text-center">
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Inherent Risk</div>
          ...
          <div className="bg-green-500/10 rounded px-3 py-2 border border-green-500/20 text-center">
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Control Strength</div>
          ...
          <div className="bg-rose-500/10 rounded px-3 py-2 border border-rose-500/20 text-center">
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Residual Risk</div>
          ...
          <div className="bg-cyan-500/10 rounded px-3 py-2 border border-cyan-500/20 text-center">
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Composite Score</div>
          ...
          <div className="bg-purple-500/10 rounded px-3 py-2 border border-purple-500/20 text-center">
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Historical Loss</div>
          ...
          <div className={`rounded px-3 py-2 border text-center ${
            ...
            <div className={`text-[10px] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Rating</div>
            <div className={`text-sm font-bold ${getRatingColor(compositeScore)}`}>{getRatingLabel(compositeScore)}</div>
          </div>
        </div>

        {/* ── Risk Score Drivers ── */}
        <SectionHeader icon={<BarChart3 className="w-3.5 h-3.5" />} label="Risk Score Drivers" color="text-emerald-400" isDark={isDark} />
        <div className="space-y-1 -mt-2">
          {sortedFactors.map((f, i) => {
            const pct = Math.round(f.contribution * 100);
            const barWidth = Math.max(4, Math.min(pct * 3, 100));
            return (
              <div key={i} className={`flex items-center gap-3 py-1.5 px-3 rounded ${isDark ? 'bg-navy-800/30 border-slate-700/30' : 'bg-gray-50 border-gray-200'} border`}>
                <span className={`text-sm ${isDark ? 'text-white' : 'text-slate-900'} font-medium w-40 shrink-0`}>{f.name}</span>
                <div className={`flex-1 h-1.5 ${isDark ? 'bg-slate-700/50' : 'bg-gray-200'} rounded-full overflow-hidden`}>
                  <div
        ...
        <SectionHeader icon={<Shield className="w-3.5 h-3.5" />} label="Residual Risk Profile" color="text-rose-400" isDark={isDark} />
        <div className="bg-rose-500/5 rounded-lg px-4 py-3 border border-rose-500/15 -mt-2">
          <p className={`text-sm ${isDark ? 'text-slate-200' : 'text-slate-700'} leading-relaxed`}>
            ...
            <span className={`${isDark ? 'text-white' : 'text-slate-900'} font-semibold`}>{residualRisk.toFixed(2)}/5</span>{' '}
            {residualRisk > 3 ? (
        ...
        <SectionHeader icon={<AlertTriangle className="w-3.5 h-3.5" />} label="Remediation & Monitoring" color="text-yellow-400" isDark={isDark} />
